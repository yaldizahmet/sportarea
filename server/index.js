...
const isMinutesInWindow = (matchMin, start, end) => {
...
const matchDayAndMinutesFromRow = (match) => {
    let dayOfWeek;
...
(async () => {
    db = await (0, sqlite_1.open)({
        filename: path_1.default.join(__dirname, 'sports.db'),
...
app.post('/api/register', async (req, res) => {
    try {
        ...
        const token = jsonwebtoken_1.default.sign({ id }, JWT_SECRET, { expiresIn: '30d' });
...
app.post('/api/login', async (req, res) => {
    try {
        ...
        if (typeof user.password === 'string' && user.password.startsWith('$2')) {
            ...
            if (isPasswordValid) {
                const upgradedHash = await bcryptjs_1.default.hash(password, BCRYPT_SALT_ROUNDS);
...
app.get('/api/users/:id/availability', async (req, res) => {
    try {
        ...
        const rows = includeInactive
            ...
            : await db.all('SELECT * FROM UserAvailability WHERE userId = ? AND isActive = 1 ORDER BY dayOfWeek, startTime', [id]);
        res.json(rows);
    }
    catch (error) {
...
app.post('/api/users/:id/availability', async (req, res) => {
    try {
        ...
        const availId = (0, node_crypto_1.randomUUID)();
        await db.run('INSERT INTO UserAvailability (id, userId, dayOfWeek, startTime, endTime, isActive) VALUES (?, ?, ?, ?, ?, 1)', [availId, id, d, String(startTime).trim(), String(endTime).trim()]);
...
app.delete('/api/users/:id/availability/:availabilityId', async (req, res) => {
    try {
        ...
        if (!user)
            return res.status(404).json({ error: 'Kullanıcı bulunamadı.' });
        const r = await db.run('DELETE FROM UserAvailability WHERE id = ? AND userId = ?', [availabilityId, id]);
        if (r && r.changes === 0) {
...
app.post('/api/matches', async (req, res) => {
    try {
        ...
        const id = Date.now().toString();
        await db.run('INSERT INTO Matches (id, groupId, creatorId, date, time, location, maxPlayers, teamAName, teamBName, matchTimestamp, lockoutHours) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [id, groupId || null, creatorId, date, time, location, maxPlayers, teamAName || 'A Takımı', teamBName || 'B Takımı', matchTimestamp || 0, lockoutHours || 1]);
        if (creatorId) {
...
app.get('/api/matches/:id/suggested-players', async (req, res) => {
    try {
        ...
        const members = await db.all(`SELECT u.id, u.name, u.avatar, u.position
       FROM User u
       JOIN GroupMembers gm ON u.id = gm.userId
       WHERE gm.groupId = ?`, [match.groupId]);
        const inMatchRows = await db.all('SELECT userId FROM MatchPlayers WHERE matchId = ?', [matchId]);
        const inMatchSet = new Set(inMatchRows.map((r) => r.userId));
        const lastCompleted = await db.all(`SELECT id FROM Matches
       WHERE groupId = ? AND status = 'COMPLETED'
       ORDER BY COALESCE(matchTimestamp, 0) DESC, createdAt DESC
       LIMIT 2`, [match.groupId]);
        const lastMatchIds = lastCompleted.map((r) => r.id);
        ...
        for (const u of members) {
            if (inMatchSet.has(u.id))
                continue;
            const rows = await db.all('SELECT * FROM UserAvailability WHERE userId = ? AND dayOfWeek = ? AND isActive = 1', [u.id, dayOfWeek]);
            const inSlot = rows.some((row) => isMinutesInWindow(matchMinutes, row.startTime, row.endTime));
            let playedRecentGroupMatch = false;
            if (idPlaceholders) {
                const c = await db.get(`SELECT COUNT(*) as c FROM MatchPlayers WHERE userId = ? AND matchId IN (${idPlaceholders})`, [u.id, ...lastMatchIds]);
...
app.post('/api/matches/:id/divide', async (req, res) => {
    ...
    `, [id]);
        if (players.length < 2)
            return res.status(400).json({ error: 'Takım kurmak için yeterli oyuncu yok.' });
        ...
        const goalkeepers = players.filter((p) => (p.position || '').toLowerCase().includes('kaleci')).sort((a, b) => b.overall - a.overall);
        const others = players.filter((p) => !(p.position || '').toLowerCase().includes('kaleci')).sort((a, b) => b.overall - a.overall);
        const teamA = [];
        const teamB = [];
        let sumA = 0;
        let sumB = 0;
        const assignToTeam = (p, forceBalance = false) => {
        ...
        goalkeepers.forEach((p) => assignToTeam(p));
        // Sonra kalanları dağıt
        others.forEach((p) => assignToTeam(p));
        await db.run('UPDATE MatchPlayers SET team = "UNASSIGNED" WHERE matchId = ?', [id]);
        for (const p of teamA)
            await db.run('UPDATE MatchPlayers SET team = "A" WHERE matchId = ? AND userId = ?', [id, p.id]);
        for (const p of teamB)
            await db.run('UPDATE MatchPlayers SET team = "B" WHERE matchId = ? AND userId = ?', [id, p.id]);
        res.json({
...
app.post('/api/matches/:id/rate', async (req, res) => {
    try {
        ...
        if (raterId === ratedId)
            return res.status(400).json({ error: 'Kendinizi puanlayamazsınız.' });
...
app.get('/api/users/:id/stats', async (req, res) => {
    ...
    `, [id]);
        ...
        // Core achievements
        if (realGoals >= 5)
            badges.push({ id: 'top_scorer', icon: '⚽', title: 'Gol Makinesi', bg: 'rgba(0, 230, 118, 0.15)' });
        if (numMatches >= 10)
            badges.push({ id: 'veteran', icon: '🌟', title: 'Müdavim', bg: 'rgba(56, 189, 248, 0.15)' });
        if (mvpVotes > 0)
            badges.push({ id: 'mvp', icon: '🏆', title: 'Yıldız Oyuncu', bg: 'rgba(255, 193, 7, 0.15)' });
        if (overallScore >= 75)
            badges.push({ id: 'pro', icon: '🔥', title: 'Pro Kariyer', bg: 'rgba(239, 68, 68, 0.15)' });
        // Fun and dynamic badges
        if (realGoals >= 15)
            badges.push({ id: 'golden_boot', icon: '👑', title: 'Altın Ayakkabı', bg: 'rgba(250, 204, 21, 0.15)' });
        if (numMatches >= 25)
            badges.push({ id: 'legend', icon: '🪐', title: 'Halı Saha Efsanesi', bg: 'rgba(251, 146, 60, 0.15)' });
        if (physique >= 78)
            badges.push({ id: 'defense_minister', icon: '🛡️', title: 'Savunma Bakanı', bg: 'rgba(148, 163, 184, 0.15)' });
        if (pass >= 78)
            badges.push({ id: 'maestro', icon: '🪄', title: 'Sihirbaz', bg: 'rgba(168, 85, 247, 0.15)' });
        if (speed >= 78)
            badges.push({ id: 'lightning', icon: '⚡', title: 'Fırtına', bg: 'rgba(56, 189, 248, 0.15)' });
        if (shoot >= 78)
            badges.push({ id: 'sniper', icon: '🎯', title: 'Keskin Nişancı', bg: 'rgba(244, 63, 94, 0.15)' });
        if (ratingCount >= 5)
            badges.push({ id: 'people_hero', icon: '🤝', title: 'Halk Kahramanı', bg: 'rgba(20, 184, 166, 0.15)' });
        if (physique >= 82 && pass < 65)
            badges.push({ id: 'gladiator', icon: '⚔️', title: 'Gladyatör (Gattuso)', bg: 'rgba(220, 38, 38, 0.15)' });
        res.json({
            matches: numMatches,
